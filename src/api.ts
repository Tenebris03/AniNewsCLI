...
export async function fetchCurrentlyAiring(): Promise<CurrentlyAiringMedia[]> {
  let page = 1;
  let hasNextPage = true;
  const allMedia: CurrentlyAiringMedia[] = [];
  ...
  while (hasNextPage) {
    ...
    const data = (await response.json()) as AiringMediaResponse;
